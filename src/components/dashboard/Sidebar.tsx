...
const Sidebar = ({
  ...
  return (
    ...
    <aside
      className={`fixed inset-y-0 left-0 z-40 w-72 transform border-r border-slate-200/70 bg-white/80 backdrop-blur transition duration-300 ease-in-out dark:border-slate-800 dark:bg-slate-950/70 lg:static lg:translate-x-0 ${isVisible ? "translate-x-0" : "-translate-x-full"
        }`}
    >
      <div className="flex h-full flex-col gap-6 px-6 py-8">
        ...
        <nav className="flex-1 space-y-2">
          ...
          {navItems.map(({ label, key, icon: Icon, link }) => {
